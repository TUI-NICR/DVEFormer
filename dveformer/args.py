...
from nicr_mt_scene_analysis.task_helper.dense_visual_embedding import KNOWN_DENSE_VISUAL_EMBEDDING_LOSS_FUNCTIONS
...
class ArgParserDVEFormer(ap.ArgumentParser):
    def __init__(self, *args, **kwargs):
        ...
        group.add_argument(
            ...
            help="Use automatic mixed precision (AMP) for faster training. "
                 ...
                 "instabilities. The imagenet pretraining is usually done "
                 "with amp, so it might be fine to use it here as well."
        )
        # -> rgb encoder
        ...
        group.add_argument(
            ...
            choices=('disabled', 'fp16', 'bfp16'),
            help="Use automatic mixed precision (AMP) for faster training. "
                 "If fp16 or bfp16 is used, the context module will be "
        ...
        # -> dense visual embedding related parameters
        group = self.add_argument_group('Model: Decoder(s) -> Dense Visual Embedding')
        group.add_argument(
            '--dense-visual-embedding-loss',
            type=str,
            default='cos_emb',
            choices=KNOWN_DENSE_VISUAL_EMBEDDING_LOSS_FUNCTIONS,
            help="Loss function to use for dense visual embedding."
        )
        group.add_argument(
            '--dense-visual-embedding-diff-factor',
            type=float,
            default=0.65,
        ...
        group.add_argument(
            '--dense-visual-embedding-encoder-decoder-fusion',
            type=str,
            choices=KNOWN_ENCODER_DECODER_FUSIONS,
            default='swin-ln-select',
            help="Determines how features of the encoder (after fusing "
                 "encoder features) are fused into the dense visual embedding "
                 "decoder."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder',
            type=str,
            default='segformermlp',
            choices=KNOWN_DECODERS,
            help="Decoder type to use for dense visual embedding segmentation."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder-block',
            type=str,
            default='nonbottleneck1d',
            choices=KNOWN_BLOCKS,
            help="[EMSANet decoder] Block (type) to use in dense visual "
                 "embedding decoder."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder-block-dropout-p',
            type=float,
            default=0.2,
            help="[EMSANet decoder] Dropout probability to use in dense visual "
                 "embedding decoder blocks (only for 'nonbottleneck1d')."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder-n-blocks',
            type=int,
            default=3,
            help="[EMSANet decoder] Number of blocks to use in each semantic "
        ...
        group.add_argument(
            '--dense-visual-embedding-decoder-dropout-p',
            type=float,
            default=0.1,
            help="[SegFormerMLP decoder] Probability to use for feature "
                 "dropout (Dropout2d) in dense visual embedding decoder before "
                 "task head."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder-n-channels',
            type=int,
            default=(256, 128, 64, 64),
            nargs='+',
            help="[EMSANet decoder] Number of features maps (channels) to use "
                 "in each dense visual embedding decoder module. Length of tuple "
                 "determines the number of decoder modules. "
                 "[SegFormerMLP decoder] Embedding dimensions to use for main "
                 "branch and skip connections."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder-downsamplings',
            type=int,
            default=(16, 8, 4),
            nargs='+',
            help="[EMSANet decoder] Downsampling at the end of each semantic "
                 "embedding decoder module. Length of tuple must match "
                 "`--dense-visual-embedding-decoder-n-channels`."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder-upsampling',
            choices=KNOWN_UPSAMPLING_METHODS,
            default='bilinear',
            help="[EMSANet decoder] How features are upsampled in the dense "
                 "visual embedding decoders. Bilinear upsampling may cause "
                 "problems when converting to TensorRT. 'learned-3x3*' mimics "
                 "bilinear interpolation with nearest interpolation and "
                 "adaptable 3x3 depth-wise convolution subsequently."
                 "[SegFormerMLP decoder] How features are upsampled in the "
                 "dense visual embedding decoders. Only bilinear upsampling is "
                 "supported."
        )
        group.add_argument(
            '--dense-visual-embedding-reference-estimator',
            type=str,
        ...
        group.add_argument(
            '--dense-visual-embedding-decoder-n-channels-out',
            type=int,
            default=512,
            help="[SegFormerMLP decoder] Number of output channels for the "
                 "dense visual embedding decoder. This determines the "
                 "dimension of the final embedding space. "
                 "Not used for EMSANet decoder."
        )
        group.add_argument(
            '--dense-visual-embedding-decoder-n-upsamplings',
            type=int,
            default=None,
            help="[SegFormerMLP decoder] Number of additional upsamplings after "
                 "the last decoder module. The decoder will upsample to "
                 "H/4xW/4 of the input resolution and then apply n additional "
                 "upsamplings to reach the input resolution. If not specified "
                 "it will be calculated from the downsamplings of the last "
                 "decoder module. "
        )
        ...
        # -> dense visual embedding related parameters
        group.add_argument(
            '--dense-visual-embedding-no-multiscale-supervision',
            action='store_true',
            default=False,
            help="Disables multi-scale supervision for dense visual embedding "
                 "decoder."
        )
        group.add_argument(
        ...
        group = self.add_argument_group('Linear Probing')
        group.add_argument(
            '--enable-linear-probing',
            action='store_true',
            default=False,
            help="Enable linear probing by freezing the pretrained base model "
                 "and optimizing a linear head on dense visual embeddings."
        )
        group.add_argument(
            '--linear-probing-weights-path',
            type=str,
            default=None,
            help="Path to load or store linear probing head weights when "
                 "--enable-linear-probing is active."
        )
        group.add_argument(
            '--linear-probing-weights-init',
            type=str,
            default='random',
            choices=('random', 'text', 'mean'),
            help="Initialization strategy for the linear probing head."
        )
        ...
        group.add_argument(
            ...
            type=str,
            default=None,
            help="Optional thresholds to duplicate checkpoints once a metric "
                 "reaches a target value. Format: 'metric:value', e.g. "
                 "'linear_probing_miou:0.51'."
        )
        group.add_argument(
    ...
    def parse_args(self, args=None, namespace=None, verbose=True):
        ...
        # check for invalid args for linear probing
        if pa.enable_linear_probing:
            if pa.weights_filepath is None:
                _warn("Linear probing is enabled, but no weights "
                      "are provided. The base model will be randomly "
                      "initialized."
                )
            if pa.validation_only and pa.linear_probing_weights_path is None:
                _warn("Validation of linear probing is enabled, but no "
                      "'--linear-probing-weights-path' is given. "
