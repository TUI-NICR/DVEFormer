...
def load_weights(args, model, state_dict, verbose=True):
    ...
    # ensure that linear probing weights can be loaded, to still
    # allow for loading strict=True below
    if args.enable_linear_probing:
        lp_key = 'linear_probing_head.weight'
        if args.linear_probing_weights_path is not None:
            state_dict[lp_key] = \
                torch.load(
                    args.linear_probing_weights_path,
                    map_location=torch.device('cpu')
                )['linear_probing_head_state_dict']['weight']
        else:
            state_dict[lp_key] = model.linear_probing_head.weight
