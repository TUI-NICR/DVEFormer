...
from nicr_mt_scene_analysis.task_helper import DenseVisualEmbeddingTaskHelper
...
def get_task_helpers(
    ...
    if 'dense-visual-embedding' in args.tasks:
        task_helper.append(
            DenseVisualEmbeddingTaskHelper(
                n_classes=dataset.semantic_n_classes_without_void,
                loss_name=args.dense_visual_embedding_loss,
                disable_multiscale_supervision=args.dense_visual_embedding_no_multiscale_supervision,
                examples_cmap=dataset.semantic_class_colors_without_void
            )
        )
