...
def get_loss_weighting_module(args) -> LossWeightingType:
    ...
    tasks_weights = {
        # When training tasks such as dense-visual-embedding, the task helper
        # will return loss with dense_visual_embedding in the key.
        # As we don't want dashes in our loss dict, we replace it with
        # underscores, so our loss weighting module can still select the
        # correct loss.
    ...
    # Set task weights correctly when doing linear probing.
    # As we don't want any other losses, we replace the full dict.
    if args.enable_linear_probing:
        assert 'dense_visual_embedding' in tasks_weights
        tasks_weights = {
            'linear_probing': 1
        }
