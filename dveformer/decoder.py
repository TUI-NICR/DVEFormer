...
from nicr_mt_scene_analysis.model.decoder import EmbeddingDecoder
from nicr_mt_scene_analysis.model.decoder import EmbeddingMLPDecoder
...
def get_decoders(
    ...
    n_channels_in: int,
    downsampling_in: int,
    semantic_embedding_dimension: int = 768,
    ...
    mean_visual_embeddings_per_class: Optional[torch.Tensor] = None,
    **kwargs
) -> nn.ModuleList:
    ...
    if 'dense-visual-embedding' in args.tasks:
        decoder_type = args.dense_visual_embedding_decoder.lower()
        with_text_embeddings_per_class = text_embeddings_per_class is not None
        with_mean_visual_embeddings_per_class = mean_visual_embeddings_per_class is not None
        if 'emsanet' == decoder_type:
            dense_visual_embedding_decoder = EmbeddingDecoder(
                n_channels=args.dense_visual_embedding_decoder_n_channels,
                downsamplings=args.dense_visual_embedding_decoder_downsamplings,
                block=get_block_class(
                    args.dense_visual_embedding_decoder_block,
                    dropout_p=args.dense_visual_embedding_decoder_block_dropout_p
                ),
                n_blocks=args.dense_visual_embedding_decoder_n_blocks,
                fusion=get_encoder_decoder_fusion_class(args.dense_visual_embedding_encoder_decoder_fusion),
                embedding_dim=semantic_embedding_dimension,
                postprocessing=get_postprocessing_class(
                    'dense-visual-embedding',
                    with_text_embeddings_per_class=with_text_embeddings_per_class,
                    text_embeddings_per_class=text_embeddings_per_class,
                    with_mean_visual_embedding_per_class=with_mean_visual_embeddings_per_class,
                    mean_visual_embedding_per_class=mean_visual_embeddings_per_class,
                    **kwargs
                ),
                upsampling=get_upsampling_class(args.dense_visual_embedding_decoder_upsampling),
                n_upsamplings=args.dense_visual_embedding_decoder_n_upsamplings,
                **common_kwargs
            )

        elif 'segformermlp' == decoder_type:
            dense_visual_embedding_decoder = EmbeddingMLPDecoder(
                n_channels=args.dense_visual_embedding_decoder_n_channels,
                fusion=get_encoder_decoder_fusion_class(args.dense_visual_embedding_encoder_decoder_fusion),
                embedding_dim=semantic_embedding_dimension,
                downsampling_in_heads=4,
                dropout_p=args.dense_visual_embedding_decoder_dropout_p,
                postprocessing=get_postprocessing_class(
                    'dense-visual-embedding',
                    with_text_embeddings_per_class=with_text_embeddings_per_class,
                    text_embeddings_per_class=text_embeddings_per_class,
                ...
                upsampling=get_upsampling_class(args.dense_visual_embedding_decoder_upsampling),
                n_channels_out=args.dense_visual_embedding_decoder_n_channels_out,
                n_upsamplings=args.dense_visual_embedding_decoder_n_upsamplings,
                **common_kwargs
            )
        decoders['dense_visual_embedding_decoder'] = dense_visual_embedding_decoder
