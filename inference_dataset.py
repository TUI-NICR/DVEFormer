...
def main():
    ...
    if args.enable_linear_probing:
        model = DVEFormerLinearProbing(args, dataset_config=dataset.config)
    else:
        model = DVEFormer(args, dataset_config=dataset.config)
    ...
    if 'scannet-panoptic' in args.inference_output_format:
        ...
        for j, batch in tqdm(enumerate(dataloader),
            ...
            if args.enable_linear_probing:
                prediction['semantic_segmentation_idx'] = (
                    prediction['linear_probing_semantic_idx']
                )
                prediction['semantic_segmentation_score'] = (
                    prediction['linear_probing_semantic_score']
                )
                prediction[get_fullres_key('semantic_segmentation_idx')] = (
                    prediction[get_fullres_key('linear_probing_semantic_idx')]
                )
                prediction[get_fullres_key('semantic_segmentation_score')] = (
                    prediction[get_fullres_key('linear_probing_semantic_score')]
                )
                ...
                if args.inference_semantic_fallback == 'text':
                    ...
                    idx_key = (
                        'dense_visual_embedding_visual_mean_based_semantic_idx'
                    )
                    score_key = (
                        'dense_visual_embedding_visual_mean_based_semantic_score'
                    )
                else:
                ...
                if idx_key is None or idx_key not in prediction:
                    ...
                    prediction['semantic_segmentation_idx'] = prediction[idx_key]
